...
pub async fn process_buffer_with_vad<F>(url: &str, mut f: F) -> Result<(), Box<dyn std::error::Error>>
where
    F: FnMut(&Vec<i16>),
{
...
pub async fn stream_to_file(url: &str) -> Result<(), Box<dyn std::error::Error>>{
    ...
    process_buffer_with_vad(url,closure_annotated).await?;
...
pub async fn transcribe_url(url: &str) -> Result<(), Box<dyn std::error::Error>> {
