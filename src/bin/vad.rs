...
use whisper_rs_test::{convert_file_to_wave, utils, vad_iter, silero};
...
*/
/*
fn old() -> Result<(), Box<dyn std::error::Error>> {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let content = convert_file_to_wave(input_file,target_sample_rate)?;

    assert!(!content.is_empty());

    // Silero VAD models were trained using 512, 1024, 1536 samples for 16000 sample rate and 256, 512, 768 samples for 8000 sample rate.
    let silero = silero::Silero::new(sample_rate, model_path).unwrap();

    let vad_params = utils::VadParams {
        threshold: 0.5,
        min_speech_duration_ms: 250,
        frame_size: 64, // 64 * 16000 / 1000 = 1024
    ...
    for timestamp in vad_iterator.speeches() {
        println!("{:?}", (timestamp.start as f32 / target_sample_rate as f32, timestamp.end as f32 / target_sample_rate as f32));
    }
    println!("Finished.");
    Ok(())
}